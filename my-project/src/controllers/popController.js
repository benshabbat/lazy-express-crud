...
export const getPopById = async (req, res) => {
    try {
        const item = await Pop.findById(req.params.id);
        if (!item) {
...
export const updatePop = async (req, res) => {
    try {
        const { name, description } = req.body;
        const updatedItem = await Pop.findByIdAndUpdate(
            req.params.id, 
            { name, description },
...
export const deletePop = async (req, res) => {
    try {
        const deleted = await Pop.findByIdAndDelete(req.params.id);
